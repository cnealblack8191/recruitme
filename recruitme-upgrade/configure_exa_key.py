...
def main():
    ...
    key=getpass.getpass('Exa API key (hidden): ').strip()
    if not 8<=len(key)<=512 or any(c.isspace() for c in key):
        raise SystemExit('Invalid key format; nothing saved.')
    os.umask(0o077)
    destination='/etc/recruitme/exa-api-key'
    if args.replace:
        group=grp.getgrnam('recruitme').gr_gid
        fd,temporary=tempfile.mkstemp(prefix='.exa-key-',dir='/etc/recruitme')
        try:
        ...
        return
    try:
        fd=os.open('/etc/recruitme/exa-api-key',os.O_WRONLY|os.O_CREAT|os.O_EXCL|os.O_NOFOLLOW,0o600)
